...
7. Check the **Select All** box. Click on Download and select the **Alginment Descrptions CSV**. Download the results in CSV format. Using the command line move the file into your data folder and rename the file to "genome01_blastn.csv".
...
1. Go to NCBI and use blastn for the for *unknown_genome_02* in the "Gene" database.  

2. Download the blast search results in CSV format, change the name to "genome02_blastn.csv" using command line and store the results in the "blast_results" folder.What commands did you use to change the name and move the file into the blast_results folder?
```{}
...
3. Read the blast results into R and clean the column names.
```{r}
...
3. What were the top 20 hits for this genome, based on Max Score?
```{r}
...
4. Which hit had the highest e-value? (include the description, taxid, e_value, and link to the accession number for this hit)
```{r}
...
3. What were the top 20 hits, based on e_value, for this blastp search?
```{r}
...
5. What were the top 20 hits, based on e_value, for this tblastn search? How do they compare to the blastp hits?
```{r}
