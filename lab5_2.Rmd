...
`geom_boxplot()` is the geom type for a standard boxplot. The center line in each box represents the median, not the mean.
```{r}
carni_mammals %>% 
...
## Practice
1. What do you think the solid lines represent? Use `count()` to determine which families have the largest and smallest sample sizes.
```{r}
carni_mammals %>% 
  count(family) %>% 
  arrange((n))
...
2. Rebuild the plot for the top five families with the largest sample sizes. How is `ggplot` ordering families on the x-axis?
```{r}
homerange %>% 
