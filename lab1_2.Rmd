...
At first, it will be hard to get used to the programming syntax- expect a learning curve. We are all working together to learn R, so let's help each other get started!
...
*Objects* in R are often referred to as *variables*, especially in other programming languages. They are not the same thing for reasons that will become apparent in the following sections.  
...
Poorly written code is a major problem in science that has received a lot of attention recently. Among the serious problems is that repeatability can break down. We must be vigilant to avoid poorly written or broken code and so will strictly follow several conventions as we progress during the quarter.

A popular data scientist named Jenifer Bryan once said: "File organization and naming are powerful weapons against chaos." We would be wise to heed her words.  

[Jennifer Bryan's talk](https://www.youtube.com/watch?v=7oyiPBjLAWY)  

## Types of Data
R is capable of working with multiple types of data. Three are common: numerics, integers, logical, and characters. Numerics include decimals and integers are generally treated as numerics unless we explicitly ask R to treat a number as an integer.
```{r}
my_numeric <- 42
...
If you ask many R programmers what is meant by 'type of data' they will often give you ways of organizing data such as data frames, vectors, lists, etc. I distinguish between the types of data and how they are organized here although in practice you will interact mostly with data frames and vectors.

## Annotations
Alhough RMarkdown allows us to thoroughly explain our code, many programmers use hashtags to annotate code chunks. These annotations can be helpful in showing how each bit of code functions and can be a helpful way to keep notes for yourself as you work.
```{r}
...
In order to make vectors more helpful, we can assign elements to names.
```{r}
...
Nice, but annoying. Nobody is going to type all of this in over and over again. If you build the classification once then it can be reused. Just make sure the vector is the same length.
```{r}
scientists_2 <- c("Sagan", "physicist", "Male", "Lyell", "geologist", "Male", "Franklin", "biologist", "Female", "McClintock", "biologist", "Female")

classification <- c("Name", "Profession", "Sex", "Name", "Profession", "Sex", "Name", "Profession", "Sex", "Name", "Profession", "Sex")

names(scientists_2) <- classification

scientists_2
```

Let's combine them. Remember, this isn't addition so we need to build a new object and another vector.
```{r}
scientists_total <- c(scientists, scientists_2)
scientists_total
```

Separate the sexes of the scientists. Notice that because this is a vector, we have to specify the position of each element. This makes working with character vectors inefficient; they are better suited to numericals.
```{r}
scientists_sex <- scientists_total[c(3, 6, 9, 12, 15, 18, 21, 24)]

scientists_male <-scientists_sex[c(1,2,3,5,6)]
scientists_male

scientists_female <- scientists_sex[c(4, 7, 8)]
scientists_female
```

You can also use the [] to call named elements. This makes things a bit easier.
```{r}
scientists_male2 <- scientists_total[scientists_total == "Male"]
scientists_female2 <- scientists_total[scientists_total == "Female"]
males <- length(scientists_male2)
females <- length(scientists_female2)
males>females #is the number of male scientists larger than fremale scientists?
```
...
We can evaluate all numerics in our vector to determine which has a value of 15.
```{r}
new_vector == 15
```

Now we are getting somewhere, because we can use <, >, ==, <==, etc.
```{r}
new_vector < 10
```
...
If you use the [] then you will only get the values, not the logical evaluation of the entire vector.
```{r}
new_vector[new_vector < 10 & new_vector > 2]
```

We will come back to vectors later. In practice, most data in R is organized into data frames; i.e. data tables. Data frames operate  like a series of equal length vectors, so it is helpful to know how to work with both.
