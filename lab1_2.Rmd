...
Poorly written code is a major problem in science that has received a lot of attention recently. Among the serious problems is that repeatability can break down. We must be vigilant to avoid poorly written or broken code and so will follow several conventions as we progress during the quarter.

A popular data scientist named Jennifer Bryan said: "File organization and naming are powerful weapons against chaos." We would be wise to heed her words.  
...
In order to make vectors more helpful, we can assign elements to objects.
```{r}
