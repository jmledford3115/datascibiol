---
title: "Tidyverse: filter, select, pipes, and mutate"
author: "Joel Ledford"
date: "Winter 2019"
output:
  html_document:
...
## Review
In the last section you were introduced to the `tidyverse` and the core function `filter()`. Filter allows us to separate variables with specific attributes of interest from large, complex data. `filter()` works by looking through rows, but what about columns? Recall that each column is essentially a vector, so we need the new function `select()` to separate columns of interest. As you may expect, we can also use them together to focus our data on the variables and values of interest.
...
Use `mutate()` to calculate the half length of each fish: length_half = length/2. Select only fish_id, length, and length_half, use pipes.
```{r}
fish %>% 
