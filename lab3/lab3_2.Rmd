...
`table()` is useful when you have a limited number of categorical variables. We will come back to this later... 
```{r}
#table(fish$lakeid)
```
...
6. Try the `table()` command to produce counts of mammal order, family, and genus.  
```{r}
