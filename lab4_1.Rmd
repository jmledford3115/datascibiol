...
Here we add a new column with Year, which are stored as rownames above. `mutate()` is a tidyverse verb we have not discussed yet, but it is used to create new variables. The USPersonalExpenditures data also needs to be converted to a data frame before we can use the tidyverse functions, because it comes as a matrix.
```{r}
expenditures <- 
  USPersonalExpenditure %>% 
  as.data.frame() %>% 
...
expenditures_tidy <- 
  expenditures %>% 
  gather(`1940`, `1945`, `1950`, `1955`, `1960`, key = "Year", value = "Billions")
expenditures_tidy
```
