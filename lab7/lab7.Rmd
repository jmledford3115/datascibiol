...
## Review
`summarize()` is a function in `dplyr` that allows us to produce summaries of data. It's output is a new dataframe. If we were interested in the mean body weight of _Adeile_ penguins, we could use `summarize()`. Do this below, please.
```{r}
...
What if we wanted to summarize basic statistics for each species? 
```{r}
