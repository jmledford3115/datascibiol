...
Let's load the mammals life history data and clean the names using `janitor.`
```{r}
...
2. What are the possible problems if missing data are indicated by values such as "-999.0" or "0"?  


3. What should the plan be for dealing with NAs in the `life_history` data?  

## Replacing values with NA
If you are certain that NAs are represented as values in the data, then the best way to deal with them is to replace them. Although you can do this when you import the data, in most cases you will need to do some exploration first.  

`naniar` has an easy function to make this happen and it is very handy. Run the code below and then have another look at the data.
```{r}
life_history %>% 
...
Let's say we want to treat "domesticated" as NA in the conservation column. For this, we use `mutate()` and `na_if()`.
```{r}
msleep %>%  
