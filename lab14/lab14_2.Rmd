...
plot(out.em2)
plot(out.em2, chr=c(7,15), ylab="LOD Score")
```
### iplotScanone
...
```{r}
iplotScanone(out.em)
iplotScanone(out.em2, lodcolumn = 2)
```
...
The object phe is an 162×241 matrix: curves for 162 individuals at 241 time points
```{r}
times1 <- as.numeric(sub("T", "", phenames(grav)))/60
str(times1)
times <- attr(grav, "time") #set specific attributes of an object.
str(times)
#?attr
head(grav$pheno)
phe <- grav$pheno
head(phe)
```
The values on the x-axis are simply column indices. To use the actual values in times, include it as a second argument.
```{r}
...
More interesting is to have the curves linked to scatterplots, of the response at particular times.
```{r}
...
This is done by providing two-column matrices whose rows correspond to the main matrix.
```{r}

```

You can also include a numeric vector of groups assignments for the curves, to have groups of curves and points with common colors. For example, we can pull out the genotype for a marker and color the curves by their genotype. We’ll use the fill.geno function in R/qtl to impute any missing genotypes.
```{r}

```

To change the colors that are used in the plot, you need to provide a vector of colors with the chartOpts options `color` and `colorhilit`.
```{r}
...
```{r}
grav <- calc.genoprob(grav, step=1) # 1cM
out.em <- scanone(grav) # By default, it considers the first phenotype
out.em <- scanone(grav, pheno.col=1:nphe(grav)) # all phenotype
summary(out.em)
```
```{r}
?iplotScanone
iplotScanone(out.em, grav) # By default, it considers the first phenotype
iplotScanone(out.em, grav, lodcolumn=1)
iplotScanone(out.em, grav, lodcolumn=60)
```
...
for(i in 1:length(times_m))
    print(iplotScanone(out.em, grav, lodcolumn=times_number[i], chartOpts=list(title=times_clear[i])))
```
