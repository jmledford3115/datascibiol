...
##Review
Now that you have been introduced to the grammar of graphics using `ggplot`, we need to learn how to manipulate the plots to better suit our aesthetic needs. Aesthetics can make a significant difference visually, but you can take it too far so remember that our goal is to produce clean plots that are not too distracting.  
...
This brings us to the built-in themes which are quick ways to adjust aesthetics for a uniform look. There are many built-in themes and most of the internal aesthetics are adjustable. Take a few moments to try out different examples.
```{r}
?theme_linedraw
```
...
Barplots use the same aesthetics as scatterplots, but there are some interesting differences. This page from the [R Cookbook](http://www.cookbook-r.com/Graphs/Bar_and_line_graphs_(ggplot2)/#bar-graphs) is helpful as I routinely get confused by the different types of barplots.  
...
##Boxplots & Practice
As with scatterplots and barplots, the aesthetics for boxplots are the same. We won't use the grouped data in this case because boxplots work by plotting distributions of data. Which African countries have the highest and lowest average life spans between 1952-2007?  
