...
## Review
In the last section you were introduced to the `tidyverse` and the core function `filter()`. Filter allows us to separate specific variables of interest from large, complex data. `filter()` works by looking through rows, but what about columns? Recall that each column is a vector, so we need the new function `select()` to separate columns of interest. As you may expect, we can use `filter()` and `select()` together to build dataframes focused on our variables of interest.
