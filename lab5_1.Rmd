...
`stat="identity"` allows us to map a variable to the y axis so that we aren't restricted to count.  

First, practice your skills by filtering out carnivorous mammals.
```{r}
carni_mammals <- 
  homerange %>% 
...
Now, summarize mean body weight by family.
```{r}
carni_mammals_mass <- 
  carni_mammals %>% 
  group_by(family) %>% 
...
Now let's plot the data using `stat="identity"` to visualize mean body weight by family.
```{r}
carni_mammals_mass %>% 
...
This looks nice, but the x axis is a mess and needs adjustment. We can adjust the angle of the x-axis text by using `theme()`.
```{r}
carni_mammals_mass%>% 
  ggplot(aes(x=family, y=mean_body_wt))+
  geom_bar(stat="identity")+
...
1. Are there more herbivores or carnivores in mammals? Make a bar plot that shows their relative numbers.
```{r}
mammals %>% 
  ggplot(aes(x=trophic.guild))+
  geom_bar(stat="count")
...
2. Is there a positive or negative relationship between mass and homerange? How does this compare between herbivores and carnivores? Provide examples below.  
