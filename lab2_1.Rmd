...
## Setup
At the beginning of each class, please make sure that you navigate to the class GitHub and download the `class_files` folder. In our class GitHub, you will see a directory called `class_files`. Click on the `Clone or download` button and download the files as a `.zip`. You should now have a folder called `class_files` on your computer inside of which are all of the files that we will use for today's class.  
...
Box office earnings for Star Wars movies (in millions!). Notice that these are vectors.
```{r}
...
Construct star_wars_matrix using the `matrix()` command. We need to tell R how to organize the `box_office` vector using the `nrow` and `byrow` commands.
```{r}
...
The same methods of selecting elements in a vector apply. We use `[]`.
```{r}
...
2. Build a data matrix that has the columns named by scientist and rows by the names of the springs.
```{r}
...
3. Let's name the columns and rows. Start by making two new vectors with the names.
```{r}
