...
## Data Matrix
Last time, you learned how to work with vectors. In the homework assignment, you had a preview of a data matrix when you combined data about genome sizes in several organisms. Although we used the data.frame command, technically speaking we built a data matrix because a data matrix is restricted to *data of the same type*. In the short example (from DataCamp) below, we will build a new data matrix using the matrix command.
...
Calculate the mean of all three scientists and add it as a new column.
```{r}
...
## Final Thoughts on Data Matrices
Working with data matrices requires all of the data to be of the same type. For some applications (linear algebra) they are routinely used and many packages require data to be stored as a matrix. Many programmers would also point out that data matrices are more efficient when compared to data frames (see below), especially for large data sets. This is great in principle, but in practice most biological data are heterogeneous and thus must be stored as a data frame. Lastly, nearly all of the packages that we will use in R only work with data frames. We will work with data frames almost exclusively for the remainder of the course.

## Data Frames
A data fram is a fancy way of saying data table with one caveat; we need to bear in mind that the data in a frame can be of multiple types as defined by R. This should make good sense as we often want to analyze relationships between numerical values, logical values, lists, etc. and this isn't possible in a data matrix.

In most respects, working with data frames is just easier.
...
Let's go back to the homework example. Here is the list of organisms.
```{r}
...
## Data matrix vs. Data Frame
Take a moment to reflect on the differences between these two data structures. What are the primary differences between them? Which do you think will be most relevant to the type of data you expect to work with in your field?
