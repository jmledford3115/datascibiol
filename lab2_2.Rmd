...
## Data Frames
For the remainder of the course, we will work with data frames. Recall that data frames allow us to store multiple types of data, so they have advantages over both vectors and data matrices. In the first session, you were shown how to build data frames using the data.frame() command. However, most scientists make their data available in some form as supplementary material in a publication. As data scientists, we can capitalize on this by importing data directly into R.
...
getwd()
```
It is best to store the data as a new object. My only caveat is that unless you really need to, don't create an object called 'data' or 'dat'. Other bad choices include: x, y, z, input, etc. Transparency: I am guilty of all of these!  

Also, while many people specify a discrete path in the read command it only works on their computer because it is local. This is generally not good practice. Instead include the .csv file in your working directory.

Here we open a .csv file with data on fish scales. Since we are using the tidyverse, we open the file using read_csv. In non-tidyverse R you would use the read.csv command. There are a few important differences between read.csv and read_csv, but for now it is enough to know that we are learning R the 'tidy' way.
```{r message=FALSE, warning=FALSE}
...
## Practice
Load the data frame mammal life histories and place it into a new object.
```{r message=FALSE, warning=FALSE}
...
The tidyverse is a collection of packages that make workflows in R easier. It was originally developed by Hadley Wickham who is, for all practical purposes, the wizard of the R universe. The packages operate differently than base R commands so if you look at older code you will see that there are many different ways of accomplishing the same task. Although not universal, the tidyverse has been adopted by many users. In our class, we embrace the tidyverse philosophy and thus will be doing things the tidy way.  
...
Notice that the code below generates a data frame larger than above because it includes non AL lakes.
```{r}
filter(fish, lakeid=="AL" | length<350)
```

If you want to filter among multiple values in the same variable you can use the %in% operator. This is a useful way to simnplify your code.
```{r}
